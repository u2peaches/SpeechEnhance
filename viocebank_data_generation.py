...
def wav_split(wav, win_length, strid):
    ...
    if len(wav) > win_length:
        ...
        slices.append(wav[-win_length:])  # wav语音最后一段不足由前面一部分进行补充
    return slices
...
def save_slices(slices, name):
    ...
    if len(slices) > 0:
        for i, slice_wav in enumerate(slices):
            ...
            np.save(name_slice, slice_wav)  # 对分段的数据进行存储
...
if __name__ == "__main__":
    ...
    # 未处理数据集存放位置
    clean_wav_path = "dataset/voicebank/clean_trainset_wav/"
    noisy_wav_path = "dataset/voicebank/noisy_trainset_wav/"

    # 处理后数据集存放位置
    catch_train_clean = "dataset/segan/voicebank/clean/"
    catch_train_noisy = "dataset/segan/voicebank/noisy/"
    ...
    # train_segan.scp用于存储处理好后的clean语音以及noisy语音对
    with open("dataset/train_segan.scp", 'wt') as f:
