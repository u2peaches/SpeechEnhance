    ...
    for clean_file in test_clean_wavs:
        ...
        for root, dirs, files in os.walk(path_test_noisy):
            ...
            if not os.path.exists(file_noisy_name):
                continue
            # 读取干净语音
            clean, fs = librosa.load(clean_file, sr=16000)
            # 读取读取噪声语音
            noisy, fs = librosa.load(file_noisy_name, sr=16000)
            ...
            sf.write(os.path.join(path_eval, 'denoised_wav/', snr, 'enh-' + name), enhance, fs)
            ...
            # plt.subplot(3, 1, 3)
            # plt.specgram(enhance, NFFT=512, Fs=fs)
